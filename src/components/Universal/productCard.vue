<template>
  ...
  >
    <div
      class="absolute z-20 top-2 right-2 flex flex-col gap-1 lg:opacity-0 lg:-translate-y-4 lg:group-hover:opacity-100 lg:group-hover:translate-y-0 transition-all duration-300"
    >
      <span
      ...
      >
        ...
        />
        <div
          class="hidden lg:block absolute right-full mr-3 top-1/2 -translate-y-1/2 px-2 py-1 bg-dark text-white text-[10px] rounded whitespace-nowrap opacity-0 -translate-x-2 pointer-events-none transition-all duration-300 group-hover/icon:opacity-100 group-hover/icon:translate-x-0 z-50"
        >
    ...
    <div class="flex flex-col gap-1 min-h-20 relative">
      <div class="relative h-6 overflow-hidden">
        <p
          class="absolute inset-0 transition-all duration-500 lg:group-hover:opacity-0 lg:group-hover:-translate-y-full text-sm font-medium truncate"
        >
          {{ product.name }}
        </p>
        <span
          @click.stop="adding(product)"
    ...
    <ModalPop v-if="quickViewProduct" :value="true" @close="quickViewProduct = null">
      <div class="flex flex-col md:flex-row w-full max-h-[90vh] overflow-y-auto">
        <div class="w-full md:w-1/2 bg-gray-100">
          <img :src="quickViewProduct.image" class="w-full h-full object-cover" />
        </div>
        <div class="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-center">
          ...
          <p class="text-xl text-red-500 font-bold mb-4">${{ quickViewProduct.price }}</p>
          <p class="text-gray-600 text-sm mb-6">{{ quickViewProduct.description }}</p>
          <div class="flex justify-between items-baseline-last">
            <span
              class="flex justify-center border-b border-b-text text-dark [&_button]:cursor-pointer [&_button]:p-4 [&_button]:hover:text-text"
            >
              <button class="cursor-pointer" @click="decrementLocalQty">-</button>
              <input class="text-center w-7" v-model="quickViewQty" readonly />
              <button class="cursor-pointer" @click="incrementLocalQty">+</button>
            </span>
            <button
              @click="addingFromSearch(quickViewProduct)"
              class="bg-dark text-white py-3 px-3 cursor-pointer rounded hover:bg-gray-800 transition-colors"
            >
              Add to Cart
            </button>
          </div>
    ...
    <ModalPop v-if="selectedProduct" :value="true" @close="selectedProduct = null">
      <div class="flex flex-col md:flex-row w-full max-h-[90vh] overflow-y-auto">
        <div
          class="w-full md:w-1/2 flex flex-col items-center justify-center p-4 border-b md:border-b-0 md:border-r"
        >
        ...
        <div class="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-center gap-3">
          <h2 class="text-xs text-text uppercase">
            There are {{ cart.count }} items in your cart.
          </h2>
          <p class="text-lg text-dark font-bold">Total: ${{ Number(cart.cartTotal).toFixed(2) }}</p>
          <button
            @click="selectedProduct = null"
            class="w-full border border-dark px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors"
          >
            CONTINUE SHOPPING
          </button>
          <router-link
            :to="{ name: 'cart' }"
            class="w-full border border-dark bg-dark text-white px-4 py-2.5 text-sm text-center"
          ...
          <button
            @click="checkOut"
            :disabled="!isAgree"
            class="bg-dark text-white py-3 rounded transition-all disabled:opacity-40"
          >
            PROCEED TO CHECKOUT
          </button>
        </div>
...
const addingFromSearch = async (product) => {
  try {
    await cart.addToCart({...product, quantity: quickViewQty.value});
